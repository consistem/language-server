...
async function getMethodSignatureDetails(
	...
	if (calleeToken.l !== ld.cos_langindex || ![ld.cos_method_attrindex, ld.cos_mem_attrindex].includes(calleeToken.s)) {
		return null;
	}

	const memberRange = Range.create(parenLine, calleeToken.p, parenLine, calleeToken.p + calleeToken.c);
...
export async function getClassMethodHover(
	...
	if (position.line < startPos.line || (position.line === startPos.line && position.character < startPos.character)) {
	...
	const docContent = buildRoutineDocumentation(signature, activeIndex, { context: "hover", showHeaderInHover: false });
